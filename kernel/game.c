...
{
	clear_screen();
	print("Hello! Welcome to my mini operating system.\n");

game_menu:
	print("now,you can play game in this system.This system currently only supports one game \n 1.kill the monster. \n");
	while (1)
	{
		game_select_start = 1;
		if (select_on != 0)
...
unsigned int getkey()
{
	...
	{
		select_on = 0;
		unsigned int scan_code = (unsigned int)currentword;
		currentword = 0;
		return scan_code;
	}
}
