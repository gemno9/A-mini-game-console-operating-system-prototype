...
{
	int errno = 0;
	char string[]="Hello!Welcome to my Mini operating system£¡\n";
...
//	regs->rcx = 0xa00000;	//RSP
	...
	regs->ds = 0;
	regs->es = 0;
	print("do_execve task is running\n");
...
unsigned long init(unsigned long arg)
{
	...
	print("init task is running,arg:%#018lx\n",arg);
...
unsigned long do_fork(struct pt_regs * regs, unsigned long clone_flags, unsigned long stack_start, unsigned long stack_size)
{
	...
	print("alloc_pages,bitmap:%#018lx\n",*memory_management_struct.bits_map);
	...
	print("struct task_struct address:%#018lx\n",(unsigned long)tsk);
...
{
	print("exit task is running,arg:%#018lx\n",code);
	while(1);
}
...
inline void __switch_to(struct task_struct *prev,struct task_struct *next)
{
	...
	print("prev->thread->rsp0:%#018lx\n",prev->thread->rsp0);
	print("next->thread->rsp0:%#018lx\n",next->thread->rsp0);
}
